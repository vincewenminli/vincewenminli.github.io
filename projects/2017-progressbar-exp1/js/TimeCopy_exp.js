 ...
 */
var literature=new Array(36);
var loading_symbol=new Array(36);
var loading_symbol_name=new Array(36);
var time_length=new Array(36);
var timelist=new Array(3);//5\10\15s三者順序，timelist表示的是順序123
var giflist=new Array(12);
...
function initial_condition () 
{
	...
	{		
		...
		{
			literature[i]=file.concat((i-17).toString(),suffix);		
		}
	}
	//初始进度条
	for (var lo=0;lo<12;lo++)
	{
		if (lo<6) 
		{
			giflist[lo]=file1.concat((lo+1).toString(),suffix1);
		} 
		else{
			giflist[lo]=file2.concat((lo-5).toString(),suffix2);
		}
	}
    for (var pa=18;pa<36;pa++) 
    {
	  abstract[pa]=abstract[pa-18];
    }
}
...
function shuffle_row () {
	//对矩阵元素洗牌,以行为单位打乱
	var row,time_t,loading_t,po=36;
	while (po){
		//取一个0-po之间的随机位置或称之为随机行，不包括po,不败包括36
		row=Math.floor(Math.random()*po--)
		//将该行的所有列元素均与剩余元素序列的最后一行元素进行交换
		time_t=time_length[po];
		time_length[po]=time_length[row]
		time_length[row]=time_t;
		
		loading_t=loading_symbol[po];
		loading_symbol[po]=loading_symbol[row];
		loading_symbol[row]=loading_t;
	}
}
/*
 * 登陆页面跳转
...
var exp_id=0;
var scale_like=new Array(36);
var copy_time=new Array(36);
var scale_time=new Array(36);
